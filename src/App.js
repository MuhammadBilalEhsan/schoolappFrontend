...
import {
	BrowserRouter as Router, Switch, Redirect, useHistory,
} from "react-router-dom";
import {
	curUserFun, getUsers, getCourseFunc, getStudentCourseFunc, addNewCourseForAdmin,
	updateCourses, updateCurrentCourse, updateAllAssignments, editAvailAbleCourses,
	UpdatecurrentConversation, allCoursesRedux, updateSingleUser, newUserAdded, logoutFunc
...
const App = () => {
	...
	useEffect(async () => {
		...
		try {
			...
			let user = await axios.get(`${appSetting.severHostedUrl}/user/currentuser/${LS?.id}`, {
			...
			if (user) {
				...
				if (currentUser.isAdmin) {
					setIsAdmin(true)
					let requestForUsers = await axios.get(`${appSetting.severHostedUrl}/user/getdata`, {
						headers: { Authentication: `Bearer ${LS?.token}` }
					})
					if (requestForUsers) {
						const allUsers = requestForUsers.data.users.filter(user => user._id !== currentUser?._id)
						dispatch(getUsers(allUsers));
						const requestForCourses = await axios.get(`${appSetting.severHostedUrl}/course/getall`, {
							headers: { Authentication: `Bearer ${LS?.token}` }
						})
					...
					const myCourse = await
						axios.post(`${appSetting.severHostedUrl}/course/mycourse`,
					...
					const studentCourses = await axios.post(`${appSetting.severHostedUrl}/course/forstudent`,
		...
		socket.on("CHANGE_IN_USER", (user) => {
			if (user._id === me?.id) {
				if (user.blocked) {
					...
					dispatch(logoutFunc())
					// axios.get(`${appSetting.severHostedUrl}/user/logout`, {
					// 	headers: { Authentication: `Bearer ${LS?.token}` }
					// })
					// .then(() => {
					setNowLogin(false)
		...
		<Router>
			<Switch>
				<PrivateRoute
					...
					isAdmin={isAdmin}
					path="/dashboard"
					AdminComp={<Dashboard setAuth={setAuth} curUser={curUser} setNowLogin={setNowLogin} setIsAdmin={setIsAdmin}
						Component={<DashBoardBox currentUser={curUser} key="dashboard" />} />}
					SuccessComp={<Dashboard setAuth={setAuth} curUser={curUser} setNowLogin={setNowLogin} setIsAdmin={setIsAdmin}
				...
				<PrivateRoute
					...
					SuccessComp={<Redirect to="/dashboard" curUser={curUser} setAuth={setAuth} />}
