...
const ClassMaterials = ({ curUser }) => {
  ...
  const currentCourse = useSelector(
    (state) => state.usersReducer.currentCourse
  );
  ...
  const enrolledStudent = currentCourse?.students?.find(
    (student) => student.id === curUser?._id
  );
  ...
  useEffect(async () => {
    try {
      const res = await axios.post(
        `${appSetting.severHostedUrl}/course/specific`,
        { id: params.id },
    ...
    <Box className={classes.class_materials}>
      ...
      {showClass ? (
        ...
        (curUser?.roll === "teacher" &&
          currentCourse?.teacher_id === curUser?._id) ||
        curUser?.isAdmin ? (
          <Box>
            {openSnack ? (
              <MuiSnacks
            ...
            <TabsComp
              ...
              tab3Label={
                curUser?.roll === "teacher" || curUser?.isAdmin
                  ? "Students"
                  : "Announcement"
              }
              panel3={
