...
function Inbox(props) {
  ...
  const allConversations = useSelector(
    (state) => state.usersReducer.allConversations
  );
  const { search } = useLocation();
  const [mobileOpen, setMobileOpen] = useState(false);
  const { window } = props;
  const [allConversationsArray, setAllConversationsArray] =
  ...
  useEffect(async () => {
    try {
      ...
      const res = await axios.get(
        `${appSetting.severHostedUrl}/user/myallconversations/${props.curUser?._id}`,
          ...
          allConversationsArray.map((chat, index) => (
            ...
            >
              <ListItemIcon>
                ...
                >
                  {chat.user1ID === props.curUser?._id
                    ? chat.user2Name[0]
                    : chat.user1Name[0]}{" "}
                </Avatar>
              </ListItemIcon>
